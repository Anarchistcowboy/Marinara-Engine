...
export function AgentsPanel() {
  ...
  return (
    <div className="flex flex-col gap-1 p-3">
      ...
      {/* ── Built-in Agents ── */}
      <PanelSection title="Built-in Agents" icon={<Sparkles size={13} />} defaultOpen>
        <div className="text-[10px] text-[var(--muted-foreground)] mb-1.5">
          Enhance roleplay, track game state, and provide guidance.
        </div>
      ...
      >
        ...
        {!regexScripts || (regexScripts as RegexScriptRow[]).length === 0 ? (
          <p className="text-[10px] text-[var(--muted-foreground)] px-1 py-2">
            No regex scripts yet.
          </p>
        ) : (
          (regexScripts as RegexScriptRow[]).map((script) => {
            const placements = (() => { try { return JSON.parse(script.placement) as string[]; } catch { return []; } })();
                ...
                >
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    {placements.map((p: string) => (
                      <span key={p} className="rounded bg-[var(--secondary)] px-1 py-0.5 text-[8px] text-[var(--muted-foreground)]">
                        {p === "ai_output" ? "AI" : "User"}
                      </span>
                    ))}
